...
class Bug0():
    def __init__(self):
        ...
        self.w_max = 1        # follow wall max angular speed
        self.v_max = 0.2        # follow wall max linear speed
    ...
    def reactive_navigation(self):
        ...
        if self.state == 1:             # go to point
            if d_error <= self.dist_thresh:     # goal found
                ...
                msg.linear.x = 0.0
            elif distance_ahead <= self.wall_dist * 1.5 :   # hit
            ...
            if abs(ang_error) <= self.ang_thresh and distance_ahead >= self.wall_dist * 2: # leave
                self.state = 1
